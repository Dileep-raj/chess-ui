...
const DropPGNBox = () => {
    return <div className="h-full border-2 border-dashed rounded-md border-zinc-500 bg-gray-800 text-gray-400 font-medium text-xl flex items-center justify-center drop-file">
        <svg className="w-8 h-8 me-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
...
const PGNInput = ({ ...props }: React.HTMLAttributes<HTMLDivElement>) => {
    ...
    return <div {...props}>
        <div className="rounded-md bg-gray-800 h-64 lg:h-80 focus-within:outline-2 focus-within:outline-gray-400" onDragEnter={handleDragInput} onDragOver={handleDragInput} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            {showDropPGNBox ?
                <DropPGNBox /> :
                <div className="p-3 h-full relative">
                    <textarea className="outline-none w-full h-full font-sans tracking-wide resize-none" autoComplete="off" spellCheck="false" name="pgnContent" id="pgnContent" placeholder="Enter your PGN text here" value={pgnText}
                        onInput={e => setPgnText(e.currentTarget.value)} />
                    <div className="absolute top-2 right-2">
                        <input className="hidden" type="file" name="pgnFile" id="pgnFile" onChange={handlePGNFileInput}
                            accept=".pgn, application/x-chess-pgn, application/vnd.chess-pgn" multiple={false} />
